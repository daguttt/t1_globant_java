...
public class Exercise4ShopInventory {
        ...
        while (isMenuOpened) {
            ...
            switch (option) {
                ...
                case "1" -> {
                    ...
                    Optional<Product> foundProduct = productList.stream().filter(product -> product.name().equals(name))
                            .findFirst();
                ...
                case "2" -> {
                    ...
                    int productIndexToBuy = InputRequester.requestAnIndexFrom(convertProductsIntoListString(),
                            String.format(
                                    "Ingresa el número del producto que quieres comprar:%n(Presiona enter para cancelar)"),
                            true);
                    ...
                    int amountToBuy = InputRequester.requestInteger(
                            String.format("Ingresa la cantidad de '%s' que quieres comprar:", productToBuy.name()))
                            .orElseThrow();

                    if (amountToBuy > productToBuy.stock()) {
                        JOptionPane.showMessageDialog(
                                null,
                                String.format("No puedes comprar '%1$d' %2$s. El stock de %2$s es '%3$d'.%n",
                                        amountToBuy, productToBuy.name(), productToBuy.stock()),
                                "Stock insuficiente",
                                JOptionPane.ERROR_MESSAGE);
                        break;
                    }

                    var updatedProduct = new Product(productToBuy.name(), productToBuy.price(),
                            productToBuy.stock() - amountToBuy);
                    productList.set(productIndexToBuy, updatedProduct);
                    JOptionPane.showMessageDialog(null,
                            String.format("¡Has comprado %d %s!", amountToBuy, productToBuy.name()));
                }
                case "3" -> {
                    if (productList.isEmpty()) {
                        JOptionPane.showMessageDialog(null,
                    ...
                    if (productToRemove.stock() > 0) {
                        JOptionPane.showMessageDialog(
                                null,
                                String.format("No puedes eliminar el producto: %s. Aun tiene stock disponible: '%d'.%n",
                                        productToRemove.name(), productToRemove.stock()),
                    ...
                    JOptionPane.showMessageDialog(null,
                            String.format("¡Producto '%s' eliminado!", productToRemove.name()));
                }
                case "4" -> {
                    if (productList.isEmpty()) {
                        JOptionPane.showMessageDialog(null,
